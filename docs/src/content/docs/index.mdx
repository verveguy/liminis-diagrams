...
There is a **[live demo](./demo/)** if you would rather see it than read about it — it
...
2. **No persistence.** `@liminis/diagrams` never writes anywhere. `C4InteractiveRenderer`
   ...
   [Recipe 3](./recipes/#recipe-3-position-persistence--the-hosts-choice).
...
| Page | Answers |
|---|---|
| [Architecture](./architecture/) | Which entry point (`.`, `./core`, `./react`, `./server`) should I install, and why? |
| [C4-PlantUML reference](./dsl-reference/) | Will my existing C4-PlantUML source parse? What gets silently stripped? |
| [Data model](./data-model/) | What does a parsed diagram / layout result actually contain? (the centerpiece) |
| [Recipes](./recipes/) | Complete, runnable examples: headless SVG rendering, embedding the interactive renderer, persisting positions. |
| [GitHub](./github-integration/) | How do I get GitHub to show a rendered C4 diagram in a markdown file? |
| [Claude Code](./claude-code-integration/) | How do I get Claude to render a real C4 diagram instead of hand-drawing one? |
