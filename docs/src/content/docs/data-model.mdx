...
It happens on **both** paths. The obvious case is the manual-position path, where a node
dragged past the top or left edge lands at a negative coordinate. The less obvious case is
the auto-layout path: dagre's own output is non-negative, but the cross-boundary alignment
pass that runs after it shifts elements *outside* a boundary to line up with their targets
inside it, and that can move them left of the origin. An element outside a
`System_Boundary` with a `Rel` to something inside is enough.

Always read `viewBoxX`/`viewBoxY` from the `LayoutResult` and use them in your
`<svg viewBox="...">`, rather than assuming `0 0`.

:::caution[This page used to say the opposite]
Until 0.1.3 this section claimed the auto-layout path "always returns `viewBoxX: 0,
viewBoxY: 0`" because "dagre never produces negative coordinates". That was wrong, and
the library believed it too: `layoutC4Diagram` hardcoded a zero origin on the auto path,
so any diagram with an element outside a boundary rendered with its left side clipped —
unreachable, because a `viewBox` is a window rather than a canvas. Both paths now compute
a real bounding box. If you pinned 0.1.2 or earlier and render your own SVG, the
`viewBoxX` you are reading may be understating the diagram.
:::
...
// viewBoxX: 0, viewBoxY: 0   (auto layout — zero *here*, because nothing in this
//                             diagram lands near the origin; not a guarantee)

const { diagram: crossing } = parseC4(
  `Person(u, "You")\nSystem_Boundary(b, "Boundary") {\n  Container(c, "Inner")\n}\nRel(u, c, "Uses")`,
);
layoutC4Diagram(crossing);
// viewBoxX: -50, viewBoxY: 0   (auto layout, element outside a boundary pushed left)
