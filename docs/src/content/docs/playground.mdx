...
import C4Playground from '../../components/C4Playground.tsx'
...
<C4Playground client:only="react" source={`Person(user, "User", "End user of the system")
...
<C4Playground client:only="react" editable={false} height="14rem" source={`Person(user, "User")
System(app, "App")
Rel(user, app, "Uses"`} />
