...
3. **Given** the DSL reference, **When** a reader looks for `@startuml`/`@enduml`, `!include`, `SHOW_LEGEND()`, and `LAYOUT_*`, **Then** the reference states plainly that these are recognized and stripped, not applied.
...
- **FR-004**: The documentation MUST state explicitly that `@startuml`/`@enduml`, `!include`, `SHOW_LEGEND()`, and `LAYOUT_*` are recognized by the parser and stripped, not applied.
