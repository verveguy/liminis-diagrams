...
- **FR-024**: `ci.yml` declares `permissions: contents: read` at workflow scope — the minimum a checkout-install-and-check job needs, and no more. A deliberate divergence from the editor's `checks` job, which declares no `permissions:` block at all and so runs under whatever default the repository or org has configured; see Resolved Decisions.
...
- **FR-020**: The checkout step in both `ci.yml` and `publish.yml` sets `persist-credentials: false` — installing dependencies runs their lifecycle scripts, so the checkout token must not remain readable in `.git/config` for that code to read. Originally scoped to `publish.yml` only, on the reasoning that the editor's own `checks` job (the only `ci.yml` job this issue has a counterpart for, A-02) has no such setting either; revised to cover both files as a deliberate security divergence from the reference — see Resolved Decisions.
...
- **`persist-credentials: false` on `publish.yml`'s checkout** (FR-020, later revised — see below) — originally *not* on `ci.yml`'s. The review comment proposed both; re-reading the editor's `ci.yml` line by line showed its `checks` job (the only job `ci.yml` here mirrors, per A-02) has no `persist-credentials: false` on its checkout at all — only `package-build`, `examples-build` and `electron-shell-e2e` do, the three jobs that pack a tarball and install it into an external fixture. `ci.yml` here has no such job, so its checkout was left at defaults, matching `checks` exactly rather than the workflow's other three jobs.
...
A third round (2026-08-23 01:25), prompted by an automated PR review finding, revisited `ci.yml`'s checkout scope on security grounds rather than reference-mirroring grounds. The finding: mirroring the editor's `checks` job explains why `ci.yml`'s checkout was left at defaults, but does not by itself mitigate the exposure — `pnpm install --frozen-lockfile` runs before any check step, and a compromised or malicious dependency's lifecycle script could read a persisted checkout token from `.git/config` for the job's duration. This had already been raised once and dismissed on the grounds that FR-020 settled the question by mirroring the reference; @verveguy overrode that dismissal directly, clarifying that FR-020's original scoping came from a question about what the editor does, not a security judgement, and that "don't improve on the reference" was written to block scope/convenience creep (extra coverage upload, a Node matrix, a `concurrency` group), not to carry forward a security gap because the source has one — noting FR-007's Codecov omission as existing precedent for a recorded deliberate divergence.

The severity is stated explicitly so it isn't overstated: this repository's `default_workflow_permissions` is `read` (confirmed via the repo's Actions API), and pnpm 10 does not run dependency lifecycle scripts without an `onlyBuiltDependencies` allowlist, which this repo has none of. Both conditions already narrow the exposure substantially. The change is **defence in depth against either condition drifting** — an allowlist entry being added, or the org default flipping to write — not remediation of a live exploitable hole. `ci.yml` now sets `persist-credentials: false` on its checkout and declares `permissions: contents: read` at workflow scope, matching `publish.yml`'s existing posture. `publish.yml` itself is unchanged: it already had both, and nothing in this finding concerned the release path. Governs FR-020 (revised to cover both files) and FR-024 (new).
...
- `handarbeit-pruefer[bot]`'s PR review finding on `.github/workflows/ci.yml:22`, and @verveguy's 2026-08-23 01:25 comment on this issue overriding the prior dismissal of it — source of FR-020's revision and FR-024
